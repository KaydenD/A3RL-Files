...
['A3PL_Vehicle_HandleInitU', {
	...
	//General vehicle inits can be added here, turn signals etc
	//containerOpened eventhandler
	_veh addEventHandler ["ContainerOpened",
	{
		...
		if (_container getVariable ["locked",true]) then
		{
			...
			["System: You can't access this unless the vehicle is unlocked",Color_Red] call A3PL_Player_Notification;
		};
	}];
	...
	if (_canTow) then
	{
		_veh call A3PL_Vehicle_Init_A3PL_F150;
	};
		...
		[_veh] spawn
		{
			...
			while {player IN _veh} do
			{
				if ((speed _veh > 16) OR (speed _veh < -16)) then
				{
					...
					["System: You can only drive this vehicle 10mph since this is a dealer vehicle, sell this car at a dealership!",Color_Red] call A3PL_Player_Notification;
				};
				uiSleep 1;
			};
		};
...
["A3PL_Vehicle_Init_A3PL_MiniExcavator",
{
	...
	_veh addEventHandler ["GetIn",
	{
		...
		if ((_veh animationPhase "Bucket") > 0.5) then
		{
			...
			//hintSilent "remove A3PL_Machinery_Pickaxe";
		};
		if ((_veh animationPhase "Jackhammer") > 0.5) then
		{
			...
			//hintSilent "remove A3PL_Machinery_Bucket";
		};
		[] spawn
		{
			forksdokeydown =
