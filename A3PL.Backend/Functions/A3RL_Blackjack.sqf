["A3RL_Blackjack_Start", 
{
	...
	if(_bet > 5000) exitWith {["Error: Max bet is $5000", Color_Red] call A3PL_Player_Notification;};
...
["A3RL_Blackjack_DealerResolve", {
	...
	if((typeName (A3RL_Player_Cards select 0)) == "STRING") then {
		...
		if(!isNil"A3RL_IsDouble") then {_bet = _bet * 2; A3RL_IsDouble = nil;};
		switch (true) do {
			case (_playerValue > 21 && _dealerValue > 21): { ["Your hand busted. You lose", Color_Yellow] call A3PL_Player_Notification; };
			case (_playerValue > 21): {["Your hand busted", Color_Red] call A3PL_Player_Notification; playSound "A3RL_Slot_Sound_Lose";};
			case (_dealerValue > 21): { ["The dealer busted. You win", Color_Green] call A3PL_Player_Notification; player setVariable ["player_cash",(player getVariable ["player_cash",0])+(_bet*2),true]; playSound "A3RL_Blackjack_Chip_Raking"; A3RL_Casino_Win = A3RL_Casino_Win + _bet; };
			...
			case (_playerValue == 21): { ["BLACKJACK!!! You win", Color_Green] call A3PL_Player_Notification; player setVariable ["player_cash",(player getVariable ["player_cash",0])+(_bet*2),true]; playSound "A3RL_Blackjack_Chip_Raking"; A3RL_Casino_Win = A3RL_Casino_Win + _bet;  };
			...
			case (_dealerValue < _playerValue): { ["Your cards have a higher value. You WIN", Color_Green] call A3PL_Player_Notification; player setVariable ["player_cash",(player getVariable ["player_cash",0])+(_bet*2),true]; playSound "A3RL_Blackjack_Chip_Raking"; A3RL_Casino_Win = A3RL_Casino_Win + _bet; };
