...
["A3RL_Gang_Invite", {
	...
	if((count _members) > _maxMembers) exitWith {[format ["You can not add another member to your group, limit of %1 members reached",_maxMembers],"red"] call A3PL_Player_Notification;};
	{
		if(_invited isEqualTo (getPlayerUID _x)) exitWith {
			...
			if(isNil "_hasGang") then {
				...
				["Gang invitation sent","green"] call A3PL_Player_Notification;
			} else {
				["This person is already in a gang.","red"] call A3PL_Player_Notification;
			};
		};
	} forEach AllPlayers;
}] call Server_Setup_Compile;
...
["A3RL_Gang_SetLead", {
	...
	if((getPlayerUID player != _gang select 1)) exitWith {
		[format ["Only the leader can appoint a new leader"],"red"] call A3PL_Player_Notification;
	};

	if((getPlayerUID player == _newUID)) exitWith {
		[format ["You're already the gang's leader"],"red"] call A3PL_Player_Notification;
	};
	...
	[format ["You've appointed a new leader for your gang"],"red"] call A3PL_Player_Notification;
}] call Server_Setup_Compile;
...
["A3RL_Gang_Capture", {
	...
	if ((currentWeapon player) == "") exitwith {["Your not brandishing any weapon","red"] call A3PL_Player_Notification;};
	if ((currentWeapon player) IN ["A3PL_FireAxe","A3PL_Shovel","A3PL_Pickaxe","A3PL_Golf_Club","A3PL_Jaws","A3PL_High_Pressure","A3PL_Medium_Pressure","A3PL_Low_Pressure","A3PL_Taser","A3PL_FireExtinguisher","A3PL_Paintball_Marker","A3PL_Paintball_Marker_Camo","A3PL_Paintball_Marker_PinkCamo","A3PL_Paintball_Marker_DigitalBlue","A3PL_Paintball_Marker_Green","A3PL_Paintball_Marker_Purple","A3PL_Paintball_Marker_Red","A3PL_Paintball_Marker_Yellow","A3PL_Predator"]) exitwith {["Ha! You call that a weapon. Dumbass","red"] call A3PL_Player_Notification;};

	_capturedTime = _obj getVariable["CapturedTime",serverTime-1800];
	if(_capturedTime > (serverTime-1800)) exitWith {["This hideout has been captured recently","red"] call A3PL_Player_Notification;};

	_gang = _group getVariable["gang_data",nil];
	if(isNil "_gang") exitWith {["You're not in a gang","red"] call A3PL_Player_Notification;};
	_gangID = _gang select 0;
	if((_obj getVariable["captured",-1]) isEqualTo _gangID) exitWith {["Your gang already controls this hideout","red"] call A3PL_Player_Notification;};

	if (Player_ActionDoing) exitwith {["You're already performing an action","red"] call A3PL_Player_Notification;};
	...
	if(Player_ActionInterrupted || !_success) exitWith {["Action canceled","red"] call A3PL_Player_Notification;};

	_obj setVariable["captured",_gangID,true];
	_obj setVariable["CapturedTime",serverTime,true];

	_gangName = _gang select 2;
	[format["%1 gang has captured a hideout!",_gangName], "yellow"] remoteExec ["A3PL_Player_Notification",-2];

	["Your gang earned $10,000 for capturing this hideout","green"] call A3PL_Player_Notification;
...
["A3RL_Gang_CapturedPaycheck", {
	...
	[format["Your gang earned $%1 for holding gangs hideouts",_win],"green"] call A3PL_Player_Notification;
	[_group,_win] call A3PL_Gang_AddBank;
}] call Server_Setup_Compile;
...
["A3RL_Gang_Created", {
	...
	[format ["You've created the gang ""%"" ",_groupName],"green"] call A3PL_Player_Notification;
}] call Server_Setup_Compile;
...
["A3RL_Gang_Delete", {
	...
	if(((getPlayerUID player) != (_gang select 1))) exitWith {
		[format ["Only the Leader can disband the gang","red"] call A3PL_Player_Notification;
	};
...
["A3RL_Gang_Kicked", {
	...
	[format ["You've been removed from your gang"],"red"] call A3PL_Player_Notification;
}] call Server_Setup_Compile;
...
["A3RL_Gang_Leave", {
	...
	if(((getPlayerUID player) == (_gang select 1))) exitWith {
		[format ["The leader of a gang can't leave"],"red"] call A3PL_Player_Notification;
	};
	...
	["You've left your gang","green"] call A3PL_Player_Notification;
}] call Server_Setup_Compile;
