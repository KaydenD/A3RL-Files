...
["A3PL_Halloween_Angelmode",
{
	...
	if (goggles player == "A3PL_Ghost_Necklace") exitwith {
		if ((random 1) <= 0.1) then
		{
			["Your Anti-Ghost necklace made an insane loud noice.",Color_Yellow] call A3PL_Player_Notification;
			...
			if (_angel distance player < 2) then {
				_triggered = true;
				["You received 10 candy from a strange angel.",Color_Green] call A3PL_Player_Notification;
			...
			["Your Anti-Ghost necklace made a weird strumbling feeling",Color_Yellow] call A3PL_Player_Notification;
		};
	};
	...
	["You were cursed by the spirit of Eerie.",Color_Red] call A3PL_Player_Notification;
	["This curse will be with you for a minute, Try and escape!",Color_Yellow] call A3PL_Player_Notification;
	["If she catches you 5 times or you don't run, your soul will be hers.",Color_Yellow] call A3PL_Player_Notification;
	...
	if ((_walkseconds == 70) OR (_hit > 5)) then {
		...
		if ((round random 10) == 6) then {
			...
			["They caught you!",Color_Red] call A3PL_Player_Notification;
		};
		{deletevehicle _x;} foreach _angelarray;
	...
	if (_passed) then {
		...
		"colorCorrections" ppEffectEnable false;
		"filmGrain" ppEffectEnable false;
		["You successfully escaped the curse, you found 25 candy laying on the floor.",Color_Green] call A3PL_Player_Notification;
	...
	if (_invehicle) then {
		...
		"colorCorrections" ppEffectEnable false;
		"filmGrain" ppEffectEnable false;
		["The angels disappeared as soon as you got in the car.",Color_Yellow] call A3PL_Player_Notification;
	};
	...
	while {(((date select 3) >= 20) OR ((date select 3) <= 7)) && (_aiMonster getvariable "health" > 0) && (_aiMonster inArea "CemeteryArea")} do
	{
		...
		if (_aiMonster getvariable "attack" select 0) then {
			...
			if ((_target distance _aiMonster) <= 2.5) then {
				...
				_target setdir 60;
				["The Guardian of the Cemetery teleported you out.",Color_Red] call A3PL_Player_Notification;
			};
		} else {
...
{
	["You blew the final hit on the guardian and received 5 candy.",Color_Green] call A3PL_Player_Notification;
