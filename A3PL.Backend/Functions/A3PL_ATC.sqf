	...
	while {player IN _veh} do
	{
		...
		if (!isNil "_active" && (_active select 1 IN ["driver_radio_settings","gunner_radio_settings"])) then
		{
			...
			{						
				...
				[format ["ATC ATIS: %1",_atcText],Color_Green] call A3PL_Player_Notification;
			};
		};					
...
["A3PL_ATC_SetAircraftInfo",
{
	...
	if (typeName _veh == "STRING") exitwith {["ATC System: Error occured trying to retrieve object name", Color_Red] call A3PL_Player_Notification;};
...
{
	disableSerialization;
	if (isNull A3PL_ATC_VAR_Selected) exitwith {["ATC System: It doesn't look like you have an aircraft selected to apply CS and Clearance", Color_Red] call A3PL_Player_Notification;};
