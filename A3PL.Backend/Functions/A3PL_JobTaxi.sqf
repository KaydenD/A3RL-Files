...
["A3PL_JobTaxi_SetFare",
{
	...
	if ((typeOf _veh) != "A3PL_CVPI_Taxi") exitwith {["System: You are not inside a taxi"] call A3PL_Player_Notification;};
	if ((driver _veh) != player) exitwith {["System: Only the driver of the vehicle can change the fare rates",Color_Red] call A3PL_Player_Notification;};
	...
	if (_invalidInput) exitwith {["System Error: One of the numbers you entered is invalid, check your fare rates and try again"] call A3PL_Player_Notification;};
	//set the rate
	_rate = format ["%1",(_newFareArray select 1)];
	if (count _rate > 2) exitwith {["System Error: Your rate per 1/5 mile is higher than 99, please lower it",Color_Red] call A3PL_Player_Notification;};
	...
	if (_extrasplus > 1000) exitwith {["System Error: Your fare per 60 seconds/stationary combined can't be more then 1000, please lower it",Color_Red] call A3PL_Player_Notification;};
	...
	["System: The new taxi rates have been set",Color_Green] call A3PL_Player_Notification;
	...
	while {player == (driver _veh)} do
	{
		...
		if (_veh getVariable ["fare_running",false]) then
		{
			...
			if (_sleepAmount > 5) then
			{
				...
				_sleepAmount = 0;
				systemChat format ["Taxi meter synced to all clients"];
			};
			...
			//debug
			systemChat format ["%1 %2 %3 %4 %5",_tFareInitial,_tFareMiles,_tFare60sec,_tFare60SecStat,_tFare];
		};
	};
}] call Server_Setup_Compile;
...
["A3PL_JobTaxi_FareStart",
{
	private ["_veh"];
	_veh = param [0,objNull];
	if (isNil {_veh getVariable ["fare",nil]}) exitwith {["System Error: Please setup the fare prices for this taxi first in your interaction wheel",Color_Red] call A3PL_Player_Notification;};
